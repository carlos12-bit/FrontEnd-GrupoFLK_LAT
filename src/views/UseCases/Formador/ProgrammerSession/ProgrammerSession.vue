    ...
    <form @submit.prevent="programarSesion">
      ...
      <div class="form-group">
        ...
        <select id="fk_modo_curso" v-model="nuevaSesion.fk_modo_curso" required>
          ...
          <option v-for="modo in modos" :key="modo.id" :value="modo.id">
            {{ modo.nombre_modo }}
          </option>
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <input
          id="fecha_session"
          type="date"
          v-model="nuevaSesion.fecha_session"
          required
        />
      </div>
...
export default {
  ...
  setup() {
    ...
    const fetchModos = async () => {
      try {
        const { data, error } = await supabase.from("modos_curso").select("id, nombre_modo");
        if (error) throw error;
        modos.value = data;
      } catch (error) {
    ...
    const programarSesion = async () => {
      try {
        const { error } = await supabase.from("ProgrammerSession").insert([nuevaSesion.value]);
        if (error) throw error;
        alert("Sesión programada con éxito.");
      } catch (error) {
        console.error("Error al programar la sesión:", error.message);
        alert("Ocurrió un error al programar la sesión.");
      }
    };
...
.session-form-container {
  ...
  margin: 0 auto;
  padding: 20px;
  background-color: #f5f5f5; /* Celeste claro */
  border-radius: 10px;
...
h2 {
  ...
  font-size: 22px; /* Cambia este valor al tamaño que desees */
}
